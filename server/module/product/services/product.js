...
exports.importCsv = async (shopId, csvFilePath) => {
  try {
    ...
    ['Name', 'Category', 'Product parent', 'Short description', 'Description', 'Price', 'Sale price'].forEach((key) => {
    ...
    // load category and map, improve performer
    const categoryNames = products.map(p => p.Category.trim());
    const categories = categoryNames.length ? await DB.ProductCategory.find({
    ...
    for (const productData of products) {
      ...
      const categoryId = _.find(categories, c => c.name.toLowerCase() === productData.Category.toLowerCase());
      if (!categoryId) {
        continue;
      }
      ...
      product.salePrice = parseFloat(productData['Sale price']);
      ...
      product.weight = parseFloat(productData.Weight) || 0;
      product.weightType = (productData['Weight type'] || '').toLowerCase() === 'kg' ? 'kg' : 'litre';
      product.volume = parseFloat(productData.Volume) || 0;
