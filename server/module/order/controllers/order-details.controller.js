    ...
    res.locals.list = {
      ...
      items: items.map((item) => {
        ...
        if (item.deliveryCompany) {
          data.deliveryCompany = item.deliveryCompany.toJSON();
        }
        if (item.deliveryDriver) {
          data.deliveryDriver = item.deliveryDriver.toJSON();
        }
...
exports.details = async (req, res, next) => {
  try {
    ...
    if (data.deliveryCompanyId) {
      const deliveryCompany = await DB.Company.findOne({ _id: data.deliveryCompanyId });
      if (deliveryCompany) {
        data.deliveryCompany = deliveryCompany.toJSON();
      }
    }

    if (data.driverId) {
      const driver = await DB.Driver.findOne({ _id: data.driverId });
      if (driver) {
...
exports.tracking = async (req, res, next) => {
  try {
    const details = await DB.OrderDetail.findOne({ trackingCode: req.params.trackingCode });
    if (!details) {
      return next(PopulateResponse.notFound());
    }
...
exports.updateStatus = async (req, res, next) => {
  try {
    const validateSchema = Joi.object().keys({
      ...
      pickUpAddressObj: Joi.object().keys({
    ...
    await Service.Order.updateStatus(subOrder, validate.value.status, validate.value.pickUpAddressObj);
    await Service.Order.addLog({
      ...
      newData: {
        ...
        pickUpAddressObj: validate.value.pickUpAddressObj
      }
    });
