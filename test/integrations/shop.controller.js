...
describe('Test Shop', () => {
  ...
  describe('Test update shop', () => {
    ...
    it('Should update shop with owner role', async () => {
      ...
      expect(body.pickUpAddress).to.have.length(2);

      const shopTest = await DB.Shop.findOne({ _id: shop._id });
      expect(shopTest.pickUpAddress).to.have.length(2);
    });
